...
2. A **governing model** — a labeled transition system (LTS) defining valid transitions
...
| **Model** | A Kripke structure (LTS) defining valid transitions |
...
| `REPOST` | Snapshot a value from another contract | `/reposts/<source_id>/users/alice.id` |
...
modal id create --name <name>      # Create named identity
modal id get --name <name>         # Get named public key
modal c set-named-id <path> <name> # Bind named identity evidence
```
