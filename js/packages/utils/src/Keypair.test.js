...
describe("Keypair", () => {
  ...
  test("should generate a valid keypair", async () => {
    ...
    expect(await keypair.publicKeyAsBase58Identity()).toEqual(
      expect.any(String)
    );
  });
  ...
  test("should serialize and deserialize keypair to/from JSON", async () => {
    ...
    expect(await deserializedKeypair.publicKeyAsBase58Identity()).toEqual(
      await keypair.publicKeyAsBase58Identity()
    );
  });
  ...
  test("should deserialize known keypass", async () => {
    const keypass = {
      ...
      private_key:
        "CAESQOXrkVunUwHzs4yfH+1e5MXyeK0PTHQMJ3Jf+Sbx4/2uACJ0W1YPpuhcvLBR9Jkkk+6wx7ur63JhgbJnHPn0QYg=",
    };
  ...
  test("should sign and verify a message", async () => {
    ...
    const isInvalid = await keypair.verifySignatureForString(
      signature,
      wrongMessage
    );
  ...
  test("should create keypair from public key", async () => {
    ...
    expect(await publicKeypair.publicKeyAsBase58Identity()).toEqual(
      publicKeyId
    );
  ...
  test.skip("should handle SSH public key conversion", async () => {
    ...
    expect(await importedKeypair.publicKeyAsBase58Identity()).toEqual(
      await keypair.publicKeyAsBase58Identity()
    );
  });
});
