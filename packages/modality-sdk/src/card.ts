...
export interface ContractCard {
  ...
  contract_id: string;
  summary: string;
  parties: Record<string, { id: string; role: string }>;
  my_role: string;
  my_rights: string[];
...
const PATTERN_ROLES: Record<string, Record<string, RoleSpec>> = {
  escrow: {
    buyer: {
      rights: [
...
export function generateCard(
  ...
  let myRole = '';
  const partiesMap: Record<string, { id: string; role: string }> = {};
  for (const [role, id] of Object.entries(intent.parties)) {
    partiesMap[role] = { id, role };
    if (id === myPublicKey) myRole = role;
  }
  ...
  const terms = intent.terms as Record<string, unknown>;
