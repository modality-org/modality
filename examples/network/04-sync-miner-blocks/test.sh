...
# Test 2: Start node1
echo ""
echo "Test 2: Starting node1..."
NODE1_PID=$(test_start_process "./01-run-node1.sh" "node1")
assert_success "test_wait_for_port 10201" "Node1 should start on port 10201"
sleep 2  # Give it time to load blocks

# Test 3: Verify node1 has blocks
echo ""
echo "Test 3: Verifying node1 has blocks..."
STORAGE_OUTPUT=$(../../../rust/target/debug/modal net storage --config ./configs/node1.json 2>&1 || true)
echo "Storage output: $STORAGE_OUTPUT" >> "$CURRENT_LOG"
assert_output_contains \
    "../../../rust/target/debug/modal net storage --config ./configs/node1.json" \
    "Total Blocks" \
...
assert_output_contains \
    "../../../rust/target/debug/modal net storage --config ./configs/node2.json" \
    "Total Blocks" \
...
# Re-syncing should either show "0 persisted" or "skipped"
if echo "$RESYNC_OUTPUT" | grep -q "persisted: 0" || echo "$RESYNC_OUTPUT" | grep -q "Skipped"; then
