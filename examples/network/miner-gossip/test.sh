...
if command -v jq &> /dev/null; then
    ...
    jq '. + {run_miner: true, status_port: 8401, initial_difficulty: 1, listeners: ["/ip4/0.0.0.0/tcp/10401/ws"], miner_nominees: ["'"$PEER_ID"'"]}' "$CONFIG_FILE" > "$TMP_FILE"
...
if command -v jq &> /dev/null; then
    ...
    PEER_ID=$(jq -r '.id' "$CONFIG_FILE")
    jq '. + {run_miner: true, status_port: 8402, initial_difficulty: 1, listeners: ["/ip4/0.0.0.0/tcp/10402/ws"], bootstrappers: ["/ip4/127.0.0.1/tcp/10401/ws/p2p/'"$MINER1_PEER_ID"'"], miner_nominees: ["'"$PEER_ID"'"]}' "$CONFIG_FILE" > "$TMP_FILE"
    mv "$TMP_FILE" "$CONFIG_FILE"
fi
...
echo "  Waiting for miner1 to mine genesis block (RandomX VM initialization takes ~10s)..." >> "$CURRENT_LOG"
MINER1_LOG="$LOG_DIR/${CURRENT_TEST}_miner1-genesis.log"
assert_success "test_wait_for_log '$MINER1_LOG' 'Successfully mined and gossipped block 0' 120" "Miner1 should mine genesis block"
...
# Wait up to 20 seconds for the race condition to appear
# With shared genesis, both miners race for block 1 - race should happen quickly
echo "  Monitoring for fork choice rejection..." >> "$CURRENT_LOG"
...
MINER1_GENESIS_BLOCKS=$(grep -c "Successfully mined and gossipped block" "$LOG_DIR/${CURRENT_TEST}_miner1-genesis.log" 2>/dev/null || echo "0")
MINER1_BLOCKS=$(grep -c "Successfully mined and gossipped block" "$MINER1_LOG" 2>/dev/null || echo "0")
