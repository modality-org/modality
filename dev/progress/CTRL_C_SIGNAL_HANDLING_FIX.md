...
**Location**: `rust/modality-node/src/actions/miner.rs:35-45`
...
**Location**: `rust/modality-node/src/node.rs:694`
...
**Location**: `rust/modality-common/src/hash_tax.rs:52-54`
...
- Updated `node.wait_for_shutdown()` to call `modality_common::hash_tax::set_mining_shutdown(true)` when Ctrl-C is received
...
2. **rust/modality-node/src/node.rs**
   - Removed errant `self.shutdown()` call from `start_networking()` (line 694)
   - Added call to `modality_common::hash_tax::set_mining_shutdown(true)` in Ctrl-C handler (line 520)

3. **rust/modality-common/src/hash_tax.rs**
...
3. Signal handler sets flags:
   - Calls `modality_common::hash_tax::set_mining_shutdown(true)`
