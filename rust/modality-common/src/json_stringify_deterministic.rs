...
pub fn stringify_deterministic(obj: &Value, opts: Option<Options>) -> String {
        ...
        match &node {
            ...
            Value::Array(arr) => {
                ...
                for (i, item) in arr.iter().enumerate() {
                    let value = deterministic(&node, &i.to_string(), item, level + 1, opts, seen)
                        .unwrap_or_else(|| (opts.stringify)(&Value::Null));
                    out.push(format!(
                        "{}{}{}",
                        indent,
                        opts.space.as_ref().unwrap_or(&"".to_string()),
                        value
                    ));
                }
            ...
            Value::Object(obj) => {
                ...
                if let Some(compare) = &opts.compare {
                    node_keys
                        .sort_by(|a, b| compare(a, obj.get(*a).unwrap(), b, obj.get(*b).unwrap()));
                } else {
                    node_keys.sort();
                }
                ...
                for key in node_keys {
                    let value =
                        deterministic(&node, key, obj.get(key).unwrap(), level + 1, opts, seen)
                            .unwrap_or_else(|| (opts.stringify)(&Value::Null)); // Changed this line to handle null values
                    let key_value = format!(
                        "{}{}{}",
                        (opts.stringify)(&Value::String(key.to_string())),
                        colon_separator,
                        value
                    );
                    out.push(format!(
    ...
    deterministic(&Value::Object(Map::new()), "", obj, 0, &opts, &mut seen).unwrap_or_default()
}
