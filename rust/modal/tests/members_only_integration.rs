...
impl TestContract {
    ...
    fn add_member_to_state(&mut self, identity: &TestIdentity) {
        ...
        self.members
            .insert(identity.name.clone(), identity.public_key_hex.clone());
    }
...
fn verify_signature(signer_hex: &str, sig_entry: &serde_json::Value, message: &[u8]) -> bool {
...
fn test_members_only_full_lifecycle() {
    ...
    assert!(
        result3,
        "Alice adding Bob should pass (she's the only member)"
    );
    ...
    let result4_fail = check_all_signed(
        &pre_carol_members,
        commit4_fail
            .get("message_hex")
            .and_then(|m| m.as_str())
    ...
    assert!(
        !result4_fail,
        "Adding Carol with only Alice's sig should fail"
    );
...
fn test_any_signed_predicate_direct() {
    ...
    assert!(
        !result,
        "any_signed should fail with only stranger's signature"
    );
}
...
fn test_membership_evolution() {
    ...
    assert!(
        result,
        "Phase 1: all_signed([alice]) should pass with alice's sig"
    );
    ...
    assert!(
        !result,
        "Phase 2: all_signed([alice,bob]) should fail with only alice"
    );

    // Both sign - should pass
    let result = check_all_signed(
        &members_v2,
        &message_hex,
        &[alice_sig.clone(), bob_sig.clone()],
    );
    assert!(
        result,
        "Phase 2: all_signed([alice,bob]) should pass with both"
    );
    ...
    assert!(
        !result,
        "Phase 3: all_signed([a,b,c]) should fail without carol"
    );
    ...
    assert!(
        result,
        "Phase 3: all_signed([a,b,c]) should pass with all three"
    );
