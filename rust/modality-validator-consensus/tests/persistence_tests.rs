...
async fn test_certificate_save_load_roundtrip() {
    ...
    model.save_to_final(&datastore).await.unwrap();
    ...
    let loaded_model = DAGCertificate::find_one_multi(&datastore, keys).await.unwrap().unwrap();
...
async fn test_batch_save_load_roundtrip() {
    ...
    let loaded_model = DAGBatch::find_one_multi(&datastore, keys).await.unwrap().unwrap();
...
async fn test_dag_recovery_from_scratch() {
    ...
    // Recover DAG
    let result = recover_dag_multi(&datastore, RecoveryStrategy::FromScratch).await.unwrap();
...
async fn test_checkpoint_creation_and_recovery() {
    ...
    for i in 0..3 {
        ...
        cert.to_persistence_model().unwrap().save_to_final(&datastore).await.unwrap();
    }
    ...
    let checkpoint = DAGState::get_latest_multi(&datastore).await.unwrap();
    ...
    let result = recover_dag_multi(&datastore, RecoveryStrategy::FromCheckpoint).await.unwrap();
...
async fn test_checkpoint_pruning() {
    ...
    let iterator = store.iterator("/dag/checkpoints/round");
    for result in iterator {
        ...
        if let Some(round_str) = parts.get(4) {
            ...
            if let Some(checkpoint) = DAGState::find_one_multi(&datastore, keys).await.unwrap() {
    ...
    DAGState::prune_old_multi(&datastore, 2).await.unwrap();
    ...
    let latest = DAGState::get_latest_multi(&datastore).await.unwrap().unwrap();
...
async fn test_hybrid_recovery_strategy() {
    ...
    let result = recover_dag_multi(&datastore, RecoveryStrategy::Hybrid).await.unwrap();
...
async fn test_consensus_metadata() {
    ...
    let mut metadata = ConsensusMetadata::get_current_multi(&datastore).await.unwrap();
    ...
    // Reload and verify
    let reloaded = ConsensusMetadata::get_current_multi(&datastore).await.unwrap();
...
async fn test_mark_certificate_committed() {
    ...
    // Mark as committed
    model.mark_committed_multi(&datastore, 0).await.unwrap();
    ...
    let reloaded = DAGCertificate::find_one_multi(&datastore, keys).await.unwrap().unwrap();
...
async fn test_batch_persistence_with_certificate_reference() {
    ...
    let loaded = DAGBatch::find_one_multi(&datastore, keys).await.unwrap().unwrap();
