...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Quorum threshold: {}",
        config.committee.quorum_threshold()
    );
    println!("  Max Byzantine: {}", config.committee.max_byzantine());
    println!(
        "  Workers per validator: {}",
        config.narwhal_config.workers_per_validator
    );
    ...
    println!(
        ...
        sequencer.pending_transaction_count().await
    );
    ...
    println!(
        "✓ Advanced to round {}",
        sequencer.get_current_round().await
    );
    ...
    match sequencer.propose_batch().await {
        Ok(Some(cert)) => {
            ...
            println!("    Round: {}", cert.header.round);
            println!(
                "    Parents: {} (references from round 0)",
                cert.header.parents.len()
            );
        }
        Ok(None) => {
        ...
        Err(e) => {
            ...
            println!(
                "    - Round 1+ requires {} parent certificates (2f+1 quorum)",
                quorum_threshold
            );
            println!("    - Single validator only has their own certificate from round 0");
            println!(
                "    - Need {} validators to provide certificates for quorum",
                quorum_threshold
            );
        }
    }
