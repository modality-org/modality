...
"#;
        ...
        let pending = serde_json::json!({
            ...
            "head": {
                ...
                "replay_bundles": {
                    "oracle_attests": {
                        "replay_bundle_json": "{\"predicate\":\"hash_matches\"}"
                    }
                }
            }
        });
        ...
        let err = processor
            ...
            .await
            .expect_err("invalid replay bundle evidence must not satisfy transition predicate");
        ...
        assert!(
            err.to_string()
                .contains("predicate mismatch for oracle_attests: bundle declares hash_matches"),
            "unexpected error: {err}"
        );
    }
...
"#;
    ...
    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_claim_mismatch() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("claim-mismatched replay bundle must not satisfy transition predicate");
        ...
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "claim-mismatched evidence must not be reported as merely unpromoted: {err}"
        );
    }
...
"#;
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("stale-key replay bundle must not satisfy transition predicate");
        ...
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "stale-key evidence must not be reported as merely unpromoted: {err}"
        );
    }

    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_wrong_contract_id() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("wrong-contract replay bundle must not satisfy transition predicate");
        ...
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "wrong-contract evidence must not be reported as merely unpromoted: {err}"
        );
    }

    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_nonpositive_timestamp() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let pending = serde_json::json!({
            ...
            "head": {
                ...
                "replay_bundles": {
                    "oracle_attests": {
                        "replay_bundle_json": "{\"predicate\":\"oracle_attests\",\"max_age_seconds\":60,\"attestation\":{\"oracle_pubkey\":\"delivery-key-v1\",\"oracle_path\":\"/oracles/delivery.id\",\"claim\":\"delivered\",\"value\":\"true\",\"contract_id\":\"c1\",\"pending_commit_hash\":\"delivery-pending\",\"timestamp\":0,\"signature\":\"sig\"}}"
                    }
                }
            }
        });
        ...
        let err = processor
            ...
            .await
            .expect_err(
                "nonpositive-timestamp replay bundle must not satisfy transition predicate",
            );
        ...
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "nonpositive-timestamp evidence must not be reported as merely unpromoted: {err}"
        );
    }

    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_empty_signature() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let pending = serde_json::json!({
            ...
            "head": {
                ...
                "replay_bundles": {
                    "oracle_attests": {
                        "replay_bundle_json": "{\"predicate\":\"oracle_attests\",\"max_age_seconds\":60,\"attestation\":{\"oracle_pubkey\":\"delivery-key-v1\",\"oracle_path\":\"/oracles/delivery.id\",\"claim\":\"delivered\",\"value\":\"true\",\"contract_id\":\"c1\",\"pending_commit_hash\":\"delivery-pending\",\"timestamp\":1700000000,\"signature\":\"\"}}"
                    }
                }
            }
        });
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("empty-signature replay bundle must not satisfy transition predicate");
...
"#;
    ...
    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_expired_timestamp() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let pending = serde_json::json!({
            ...
            "head": {
                ...
                "replay_bundles": {
                    "oracle_attests": {
                        "replay_bundle_json": "{\"predicate\":\"oracle_attests\",\"max_age_seconds\":60,\"attestation\":{\"oracle_pubkey\":\"delivery-key-v1\",\"oracle_path\":\"/oracles/delivery.id\",\"claim\":\"delivered\",\"value\":\"true\",\"contract_id\":\"c1\",\"pending_commit_hash\":\"delivery-pending\",\"timestamp\":1700000000,\"signature\":\"sig\"}}"
                    }
                }
            }
        });
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("expired replay bundle must not satisfy transition predicate");
        ...
        assert!(
            err.to_string()
                .contains("oracle_attests replay bundle attestation is too old:"),
            "unexpected error: {err}"
        );
        assert!(
            err.to_string().contains("seconds (max 60)"),
            "unexpected error: {err}"
        );
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "expired evidence must not be reported as merely unpromoted: {err}"
        );
    }
...
"#;
    ...
    #[tokio::test]
    async fn process_commit_rejects_oracle_replay_bundle_pending_hash_mismatch() {
        let datastore = Arc::new(Mutex::new(DatastoreManager::create_in_memory().unwrap()));
        let processor = ContractProcessor::new(datastore.clone());
...
"#;
        ...
        let err = processor
            .process_commit("c1", "delivery-pending", &pending.to_string())
            .await
            .expect_err("wrong-hash replay bundle must not satisfy transition predicate");
        ...
        assert!(
            err.to_string().contains(
                "oracle_attests replay bundle attestation pending_commit_hash other-pending does not match pending commit delivery-pending"
            ),
            "unexpected error: {err}"
        );
        assert!(
            !err.to_string()
                .contains("not yet promoted to local transition acceptance"),
            "wrong-hash evidence must not be reported as merely unpromoted: {err}"
        );
    }
