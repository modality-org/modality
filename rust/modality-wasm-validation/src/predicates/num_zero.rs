...
pub fn evaluate(input: &PredicateInput) -> PredicateResult {
    ...
    if num_input.value.abs() < f64::EPSILON {
        ...
        PredicateResult::failure(gas_used, vec![format!("{} is not zero", num_input.value)])
    }
}
        ...
        match rule.predicate.as_str() {
            ...
            "num_gt" => {
                if let Some(threshold) = rule.params.get("threshold").and_then(|v| v.as_f64()) {
                    if threshold >= 0.0 {
                        formulas.push(format!("!(num_zero($path) & num_gt($path, {}))", threshold));
                        satisfiable = false;
                    }
                }
            }
            "num_lt" => {
                if let Some(threshold) = rule.params.get("threshold").and_then(|v| v.as_f64()) {
                    if threshold <= 0.0 {
                        formulas.push(format!("!(num_zero($path) & num_lt($path, {}))", threshold));
                        satisfiable = false;
                    }
                }
            }
