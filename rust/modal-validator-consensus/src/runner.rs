...
use modal_datastore::models::{ValidatorBlock, ValidatorBlockMessage, Transaction};
...
pub trait ConsensusRunner: Send + Sync {
    fn peerid(&self) -> &str;
    async fn on_receive_draft_block(&self, block_data: &ValidatorBlock) -> Result<Option<Ack>>;
    async fn on_receive_block_ack(&self, ack_data: &Ack) -> Result<()>;
    async fn on_receive_block_late_ack(&self, ack_data: &Ack) -> Result<()>;
    async fn on_receive_certified_block(&self, block_data: &ValidatorBlock) -> Result<Option<ValidatorBlock>>;
...
impl ConsensusRunner for Runner {
    ...
    async fn on_receive_draft_block(&self, block: &ValidatorBlock) -> Result<Option<Ack>> {
    ...
    async fn on_receive_block_ack(&self, ack: &Ack) -> Result<()> {
        ...
        {
            let datastore = self.datastore.lock().await;
            if let Some(mut block) = ValidatorBlock::find_one(
                &datastore,
    ...
    async fn on_receive_certified_block(&self, block: &ValidatorBlock) -> Result<Option<ValidatorBlock>> {
        if !block.validate_sigs()? {
            return Ok(None);
        }
...
impl Runner {
    ...
    async fn on_receive_draft_block_from_later_round(&self, block: &ValidatorBlock) -> Result<Option<Ack>> {
        let current_round_id = {
            let datastore = self.datastore.lock().await;
            datastore.get_current_round().await?
        };

        let round_message = ValidatorBlockMessage::create_from_json(serde_json::json!({
    ...
    async fn on_receive_draft_block_from_current_round(
        ...
        if whoami == block.peer_id {
            let datastore = self.datastore.lock().await;
            let eblock = ValidatorBlock::find_one(
                &datastore,
                [
    ...
    async fn on_receive_certified_block_from_later_round(
        ...
        {
            let datastore = self.datastore.lock().await;
            ValidatorBlockMessage::from_json_object(serde_json::json!({
                "round_id": block.round_id,
                "peer_id": block.peer_id,
                "type": "certified",
                "seen_at_block_id": current_round_id,
    ...
    pub async fn speed_up_to_latest_uncertified_round(&mut self) -> Result<()> {
        ...
        while round_certified {
            ...
            let existing_certs = {
                let datastore = self.datastore.lock().await;
                ValidatorBlockMessage::find_all_in_round_of_type(&datastore, round - 1, "certified").await?
            };

            for block_message in existing_certs {
                let block_content = block_message.content.clone();
                let block = ValidatorBlock::create_from_json(block_content)?;
                {
                    let datastore = self.datastore.lock().await;
        ...
        if let Some(communication) = &self.communication {
            for peer_id in &prev_round_scribes {
                ...
                if let Some(block_data) = block_data {
                    let block = ValidatorBlock::from_json_object(block_data.to_json_object()?)?;
                    if block.validate_cert(threshold as usize)? {
                        {
                            let datastore = self.datastore.lock().await;
                            block.save(&datastore).await?;
    ...
    async fn run_round_get_prev_round_certs(&mut self) -> Result<HashMap<String, String>> {
        ...
        let existing_this_round_certs = {
            let datastore = self.datastore.lock().await;
            ValidatorBlockMessage::find_all_in_round_of_type(&datastore, round, "certified").await?
        };
    ...
    async fn run_round_create_block(
        ...
        let existing_drafts = {
            let datastore = self.datastore.lock().await;
            ValidatorBlockMessage::find_all_in_round_of_type(&datastore, round_id, "draft").await?
        };

        for draft in existing_drafts {
            let draft_content = draft.content.clone();
            let block = ValidatorBlock::create_from_json(draft_content)?;
            {
                let datastore = self.datastore.lock().await;
    ...
    async fn run_round_create_ack_monitor(&mut self, signal: Option<Arc<CancellationToken>>) -> Result<JoinHandle<std::result::Result<(), anyhow::Error>>> {
        ...
        let block = {
            let datastore = self.datastore.lock().await;
            ValidatorBlock::find_one(&datastore, HashMap::from([
...
pub async fn create_runner_props_from_datastore(
    ...
    let blocks = {
        ...
        ValidatorBlock::find_all_in_round(&datastore, 0).await?
    };
