...
impl Communication for SameProcess {
    async fn broadcast_draft_block(&mut self, from: &str, block_data: &ValidatorBlock) -> Result<()> {
        ...
        for to in peer_ids {
            self.message_queue.send(Message::DraftValidatorBlock {
                from: from.to_string(),
                to,
    ...
    async fn send_block_ack(&mut self, from: &str, to: &str, ack_data: &Ack) -> Result<()> {
        if !self.is_node_offline(from) && !self.is_node_offline(to) {
            self.message_queue.send(Message::ValidatorBlockAck {
                from: from.to_string(),
                to: to.to_string(),
                ack: ack_data.clone(),
            })?;
    ...
    async fn send_block_late_ack(&mut self, from: &str, to: &str, ack_data: &Ack) -> Result<()> {
        if !self.is_node_offline(from) && !self.is_node_offline(to) {
            self.message_queue.send(Message::ValidatorBlockLateAck {
                from: from.to_string(),
                to: to.to_string(),
                ack: ack_data.clone(),
            })?;
    ...
    async fn broadcast_certified_block(&mut self, from: &str, block_data: &ValidatorBlock) -> Result<()> {
        ...
        for to in peer_ids {
            self.message_queue.send(Message::CertifiedValidatorBlock {
                from: from.to_string(),
                to: to.to_string(),
                block: block_data.clone(),
            })?;
