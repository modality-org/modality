...
pub async fn run(opts: &Opts) -> Result<()> {
    match &opts.command {
        Command::Create { dir, output } => create_contract(dir.as_ref(), output),
        Command::Propose {
            r#type,
            from,
            to,
        ...
        Command::Accept { proposal, output } => accept_proposal(proposal, output.as_ref()),
        Command::Status { contract } => show_status(contract),
        Command::Actions { contract, agent } => show_actions(contract, agent),
        Command::Act {
            contract,
            agent,
            action,
        ...
        Command::History { contract } => show_history(contract),
        Command::Log { dir, limit, output } => show_log(dir.as_ref(), *limit, output),
        Command::Verify { model, formula } => verify_contract(model, formula.as_deref()),
        Command::Parse { file } => parse_contract_file(file),
    }
}
    ...
    let proposal = match contract_type {
        ...
        other => {
            return Err(anyhow::anyhow!(
                "Unknown contract type for proposal: '{}'. Options: escrow, service",
                other
            ))
        }
    };
...
fn parse_contract_file(file: &PathBuf) -> Result<()> {
    ...
    let contract = parse_contract(file).map_err(|e| anyhow::anyhow!("Parse error: {}", e))?;
    ...
    for (i, commit) in contract.commits.iter().enumerate() {
        ...
        println!(
            "    signed_by: {} \"{}\"",
            commit.signed_by, commit.signature
        );
                ...
                CommitStatement::Do(properties) => {
                    let props_str: Vec<String> = properties
                        ...
                        .map(|p| {
                            format!(
                                "{}{}",
                                if p.sign == modality_lang::ast::PropertySign::Plus {
                                    "+"
                                } else {
                                    "-"
                                },
