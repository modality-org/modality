...
pub struct Opts {
    #[arg(
        ...
        help = "Branch to upgrade from (testnet or mainnet)"
    )]
    pub branch: String,
...
pub async fn run(opts: &Opts) -> Result<()> {
    ...
    self_replace::self_replace(&temp_path).context("Failed to replace binary")?;
    ...
    println!();
    println!(
        "🎉 Modality has been upgraded to version {}",
        manifest.version
    );
