...
fn main() {
    ...
    let commit = env::var("MODALITY_GIT_COMMIT").ok().unwrap_or_else(|| {
        Command::new("git")
    ...
    let branch = env::var("MODALITY_GIT_BRANCH").ok().unwrap_or_else(|| {
        Command::new("git")
    ...
    println!("cargo:rerun-if-env-changed=MODALITY_GIT_BRANCH");
    println!("cargo:rerun-if-env-changed=MODALITY_GIT_COMMIT");
}
