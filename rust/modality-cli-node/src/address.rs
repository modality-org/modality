...
pub async fn run(opts: &Opts) -> Result<()> {
    ...
    // Get the peer ID
    let peer_id = config.id.as_ref().context("No peer ID found in config")?;
    ...
    // Parse peer ID once
    let peer_id_parsed: modality_node::PeerId =
        peer_id.parse().context("Failed to parse peer ID")?;
    ...
    for listener in listeners {
        ...
        addr.push(modality_node::Protocol::P2p(peer_id_parsed));
        addresses.push(addr);
    }
