...
pub async fn run(opts: &Opts) -> Result<()> {
    ...
    if let (Some(start), Some(end)) = (start_time, end_time) {
        ...
        if hours > 0.0 {
            ...
            println!(
                "    Block Rate: {:.2} blocks/hour ({:.2} blocks/min)",
                blocks_per_hour,
    ...
    if let Some(min_epoch) = epoch_counts.keys().min() {
        if let Some(max_epoch) = epoch_counts.keys().max() {
            println!(
                "    Epochs: {} (range {} to {})",
                epoch_count, min_epoch, max_epoch
            );
        }
    }
    println!();
    ...
    // Miner distribution
    println!(
        "⛏️  Miner Distribution ({} unique miners)",
        miner_list.len()
    );
    println!();
    ...
    println!(
        "    Gini Coefficient: {:.3} (0 = perfect equality, 1 = max inequality)",
        gini
    );
    ...
    if let Some((top_miner, &top_count)) = miner_list.first() {
        ...
        println!(
            "    Top Miner Dominance: {:.1}% ({})",
            dominance, display_id
        );
    }
