...
pub async fn run(opts: &Opts) -> Result<()> {
    ...
    let logs_path = config.logs_path.unwrap_or_else(|| node_dir.join("logs"));
    let log_file_path = logs_path.join("node.log");

    if !log_file_path.exists() {
        bail!(
            "Log file not found at {}. Has the node been started?",
...
fn tail_follow(path: &std::path::Path, initial_lines: usize) -> Result<()> {
    ...
    let start = if lines.len() > initial_lines {
        lines.len() - initial_lines
    } else {
        0
    };
