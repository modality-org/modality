...
pub async fn run(opts: &Opts) -> Result<()> {
    ...
    match command {
        ...
        _ => {
            ...
            println!(
                "Available commands: general, mining, blocks, block <index>, datastore-get <key>"
            );
        }
    }
...
async fn inspect_blocks(datastore_manager: &DatastoreManager) -> Result<()> {
    ...
    println!();
    println!(
        "Total Blocks: {} (Canonical: {}, Orphaned: {})",
    ...
    if !canonical_blocks.is_empty() {
        let chain_tip = canonical_blocks.iter().max_by_key(|b| b.index).unwrap();
...
async fn inspect_mining(
    ...
    if !canonical_blocks.is_empty() {
        ...
        if let Some(latest) = canonical_blocks.iter().max_by_key(|b| b.index) {
            println!(
                "Latest Block: {} (target difficulty: {})",
                latest.index, &latest.target_difficulty
            );
...
async fn inspect_block_by_index(datastore_manager: &DatastoreManager, index: u64) -> Result<()> {
    ...
    if blocks.len() > 1 {
        println!();
        println!(
            "⚠️  WARNING: {} blocks found at index {} (fork detected)",
            blocks.len(),
            index
        );
    }
