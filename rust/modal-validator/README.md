...
let validator = Validator::new_default(datastore).await?;
validator.initialize().await?;

// Get current chain tip
let tip = validator.get_chain_tip().await;
```
...
let config = ShoalValidatorConfig::new_test(4, 0);

// Create and initialize validator
let validator = ShoalValidator::new(datastore, config).await?;
validator.initialize().await?;
...
validator.propose_batch().await?;

// Get consensus state
let current_round = validator.get_current_round().await;
let committed_round = validator.get_chain_tip().await;
```
