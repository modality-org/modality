...
async fn main() -> Result<()> {
    ...
    for i in 0..=10 {
        ...
        if i == 0 {
            ...
            let hash_display = if block.hash.len() >= 16 {
                &block.hash[..16]
            } else {
                &block.hash
            };
            let peer_display = if block.nominated_peer_id.len() >= 13 {
                &block.nominated_peer_id[..13]
            } else {
                &block.nominated_peer_id
            };
            println!(
                "  ✓ Block {}: hash={}, peer={}",
                i, hash_display, peer_display
            );
        } else if i == 4 {
            println!("  ...");
        }
    ...
    let request_id = node2_swarm
        .behaviour_mut()
        .reqres
        .send_request(&node1_peer_id, request);
    ...
    if response.ok {
        if let Some(data) = response.data {
            let blocks: Vec<serde_json::Value> =
                serde_json::from_value(data.get("blocks").unwrap().clone())?;
            ...
            for block in &blocks {
                ...
                let peer_display = if peer_id.len() >= 10 {
                    &peer_id[..10]
                } else {
                    peer_id
                };
    ...
    let request_id = node2_swarm
        .behaviour_mut()
        .reqres
        .send_request(&node1_peer_id, epoch_request);
    ...
    let request_id = node2_swarm
        .behaviour_mut()
        .reqres
        .send_request(&node1_peer_id, range_request);
