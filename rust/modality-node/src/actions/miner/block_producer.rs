...
pub async fn mine_and_gossip_block(
    ...
    log::info!(
        "Mining block {} with nominated peer: {}",
        index,
        nominated_peer_id
    );
    ...
    log::info!(
        "Loaded chain with {} blocks (height: {})",
        chain.blocks.len(),
        chain.height()
    );
    ...
    let (final_hash_func, final_hash_params) =
        get_hash_config(&datastore, miner_hash_func, miner_hash_params).await;
    ...
    if index != expected_next {
        ...
        return Err(anyhow::anyhow!(
            "Index mismatch: chain expects block {} but trying to mine {}",
            expected_next,
            index
        ));
    }
    ...
    log::info!(
        "Chain ready for mining. Height: {}, Mining next index: {}",
        chain.height(),
        index
    );
    ...
    if let Some(stats) = mining_stats {
        ...
        log::info!(
            "⛏️  Block {} mined: {} attempts in {:.2}s, instant: {:.2} H/s",
            index,
            stats.attempts,
            stats.duration_secs,
            stats.hashrate()
        ...
        log::info!(
            "📊 Miner Stats: avg_hashrate={:.2} H/s, total_blocks={}, total_hashes={}",
    ...
    if mined_block.header.index != index {
        log::error!(
            "Mined block index mismatch: expected {}, got {}",
            index,
    ...
    log::info!(
        "Mined block {} (epoch {}) with hash {} and difficulty {}",
        miner_block.index,
        miner_block.epoch,
        &miner_block.hash[..16],
    ...
    if miner_block.index > 0
        && miner_block
            .index
            .is_multiple_of(ROLLING_INTEGRITY_CHECK_INTERVAL)
    {
    ...
    if miner_block.index > 0 && miner_block.index.is_multiple_of(blocks_per_epoch) {
        log::info!(
            "🎯 EPOCH {} STARTED - New target difficulty: {}",
            miner_block.epoch,
            miner_block.target_difficulty
        );
...
async fn get_hash_config(
    ...
    let genesis_params = datastore_guard
        .get_string("/network/genesis_contract_id")
        .await
        .ok()
        .flatten()
        .and_then(|contract_id| {
            futures::executor::block_on(async {
                datastore_guard
                    .load_network_parameters_from_contract(&contract_id)
                    .await
                    .ok()
            })
        });
    ...
    if let Some(params) = genesis_params {
        log::info!(
            "Using miner hash configuration from genesis contract: {}",
            params.miner_hash_func
        );
        ...
        log::info!(
            "Using miner hash configuration from node config: {}",
            hash_func
        );
...
async fn gossip_block(swarm: &Arc<Mutex<crate::swarm::NodeSwarm>>, miner_block: &MinerBlock) {
    ...
    if let Ok(json) = serde_json::to_string(&gossip_msg) {
            ...
            .publish(topic, json.as_bytes())
        {
            ...
            Err(e) => {
                log::debug!(
                    "Could not gossip block {} (no peers available): {}",
                    miner_block.index,
                    e
                );
            }
...
async fn run_integrity_check(datastore: &Arc<Mutex<DatastoreManager>>, _current_index: u64) {
    let ds = datastore.lock().await;
    match crate::actions::chain_integrity::check_recent_blocks(&ds, ROLLING_INTEGRITY_WINDOW, true)
        .await
    {
        Ok(true) => {
            log::debug!(
                "✓ Rolling integrity check passed (last {} blocks)",
                ROLLING_INTEGRITY_WINDOW
            );
        }
        Ok(false) => {
