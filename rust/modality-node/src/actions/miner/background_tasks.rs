...
use crate::actions::observer::{
    get_chain_tip_index, request_chain_info_impl, sync_missing_blocks, validate_and_cleanup_chain,
};
        ...
        while let Some((peer_id, _peer_addr)) = sync_request_rx.recv().await {
            // Check if we're already syncing with this peer
            {
                ...
                if peers.contains(&peer_id) {
                    log::debug!(
                        "Sync already in progress for peer {}, skipping duplicate request",
                        peer_id
                    );
                    continue;
                }
            ...
            log::debug!(
                "🔄 Orphan detected - checking ALL peers for heavier chains (triggered by peer {})",
                peer_id
            );
                ...
                if let Some(bp_peer_id) = bp_peer_id {
                    ...
                    match result {
                        Ok(()) => {
                            ...
                            log::info!(
                                "📡 Chain sync with peer {} completed, chain tip is now {}",
                                bp_peer_id,
                                new_tip
                            );
...
pub fn start_auto_healing_task(
    ...
    tokio::spawn(async move {
        ...
        log::info!(
            "📋 Fork recovery settings: min_peers={}, epoch_threshold={}",
            fork_recovery_min_peers,
            fork_recovery_epoch_threshold
        );
        ...
        loop {
                    ...
                    match result {
                        Ok(()) => {
                            ...
                            if new_tip != tip_before {
                                log::info!(
                                    "🔧 Auto-healing: chain changed, tip is now {}",
                                    new_tip
                                );
                            }
                            let _ = mining_update_tx.send(new_tip);
                        ...
                        Err(e) => {
                            log::debug!(
                                "🔧 Auto-healing: sync check for peer {} returned: {}",
                                peer_id,
                                e
                            );
                        }
            ...
            log::info!(
                "🔧 Auto-healing: cycle complete, waiting {} seconds",
                AUTO_HEALING_INTERVAL_SECS
            );
