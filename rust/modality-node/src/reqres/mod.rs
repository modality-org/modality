...
pub async fn handle_request(
    ...
    let response = match path.as_str() {
        "/ping" => ping::handler(Some(data.clone())).await?,
        "/inspect" => inspect::handler(Some(data.clone()), datastore_manager).await?,
        "/data/block" => reqres_data::block::handler(Some(data.clone()), datastore_manager).await?,
        "/data/block/head" => {
        ...
        "/data/miner_block/canonical" => {
            reqres_data::miner_block::list_canonical::handler(Some(data.clone()), datastore_manager)
                .await?
        }
        "/data/miner_block/epoch" => {
            reqres_data::miner_block::by_epoch::handler(Some(data.clone()), datastore_manager)
                .await?
        }
        ...
        "/data/miner_block/chain_info" => {
            reqres_data::miner_block::chain_info::handler(Some(data.clone()), datastore_manager)
                .await?
        }
        "/data/miner_block/find_ancestor" => {
            reqres_data::miner_block::find_ancestor::handler(Some(data.clone()), datastore_manager)
                .await?
        }
        "/data/miner_block/debug_index" => {
            reqres_data::miner_block::debug_index::handler(Some(data.clone()), datastore_manager)
                .await?
        }
        "/dag/sync" => dag::sync::handler(Some(data.clone()), datastore_manager).await?,
        "/contract/submit" => {
            contract::submit::handler(Some(data.clone()), datastore_manager, consensus_tx.clone())
                .await?
        }
        "/contract/push" => {
