use anyhow::Result;
use modality_datastore::models::miner::integrity::{
    detect_duplicate_canonical_blocks_multi, heal_duplicate_canonical_blocks_multi,
};
...
async fn test_duplicate_canonical_block_detection_and_healing() -> Result<()> {
    ...
    assert_eq!(
        duplicates.len(),
        2,
        "Should detect 2 indices with duplicates"
    );

    // Verify duplicate at index 1
    let dup_at_1 = duplicates
        .iter()
        .find(|d| d.index == 1)
        .expect("Should find duplicate at index 1");
    assert_eq!(dup_at_1.blocks.len(), 2);

    // Verify duplicate at index 5
    let dup_at_5 = duplicates
        .iter()
        .find(|d| d.index == 5)
        .expect("Should find duplicate at index 5");
    ...
    assert_eq!(
        ...
        5,
        "Should have 5 canonical blocks before healing (1 genesis + 2 at index 1 + 2 at index 5)"
    );
    ...
    assert_eq!(
        orphaned_hashes.len(),
        2,
        "Should orphan 2 blocks (one from each duplicate set)"
    );
    ...
    assert_eq!(
        ...
        0,
        "Should have no duplicates after healing"
    );
    ...
    assert_eq!(
        canonical_after.len(),
        3,
        "Should have 3 canonical blocks after healing (1 genesis + 1 at index 1 + 1 at index 5)"
    );
    ...
    assert_eq!(
        canonical_at_1.hash, "hash_1a_aaaa",
        "Should keep block 1a (seen first)"
    );
    ...
    assert!(block_1b_after
        .orphan_reason
        .unwrap()
        .contains("Duplicate canonical block"));
    ...
    assert_eq!(
        canonical_at_5.hash, "hash_5a",
        "Should keep block 5a (seen first)"
    );
