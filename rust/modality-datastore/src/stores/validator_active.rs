...
/// Store for active validator consensus state
pub struct ValidatorActiveStore {
    db: DB,
}

impl ValidatorActiveStore {
...
impl Store for ValidatorActiveStore {
    fn db(&self) -> &DB {
        &self.db
    }
}

impl Drop for ValidatorActiveStore {
    fn drop(&mut self) {
        let _ = self.db.flush();
    }
}
