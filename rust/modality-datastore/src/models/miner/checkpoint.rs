...
impl MinerCheckpoint {
    ...
    pub fn from_block_index(block_index: u64, block_hash: String, blocks_per_epoch: u64) -> Self {
...
impl MinerCheckpoint {
    ...
    pub async fn find_by_epoch_multi(mgr: &DatastoreManager, epoch: u64) -> Result<Option<Self>> {
        let key = format!("{}/{}", CHECKPOINT_PREFIX, epoch);

        if let Some(data) = mgr.miner_canon().get(&key)? {
            let checkpoint: MinerCheckpoint =
                serde_json::from_slice(&data).context("Failed to deserialize MinerCheckpoint")?;
    ...
    pub async fn find_all_multi(mgr: &DatastoreManager) -> Result<Vec<Self>> {
        ...
        for item in mgr.miner_canon().iterator(CHECKPOINT_PREFIX) {
            let (_, value) = item?;
            let checkpoint: MinerCheckpoint =
                serde_json::from_slice(&value).context("Failed to deserialize MinerCheckpoint")?;
    ...
    pub async fn find_up_to_epoch_multi(mgr: &DatastoreManager, epoch: u64) -> Result<Vec<Self>> {
    ...
    pub async fn find_by_block_index_multi(
        ...
        let all = Self::find_all_multi(mgr).await?;
        Ok(all.into_iter().rfind(|c| c.last_block_index <= block_index))
    }
    ...
    pub async fn delete_by_epoch_multi(mgr: &DatastoreManager, epoch: u64) -> Result<()> {
        let key = format!("{}/{}", CHECKPOINT_PREFIX, epoch);
...
pub async fn validate_block_against_checkpoints(
    ...
    let relevant_checkpoint = checkpoints
        .iter()
    ...
    while depth < max_depth {
        ...
        match MinerBlock::find_by_hash_multi(mgr, &current_hash).await? {
            Some(parent) => {
                ...
                if parent.index <= checkpoint.last_block_index
                    && parent.hash != checkpoint.last_block_hash
                {
                    log::warn!(
    ...
    // Exceeded max depth
    log::warn!(
        "Exceeded max depth tracing ancestry for block {}",
        block_index
    );
    Ok(false)
}
...
mod tests {
    ...
    async fn test_find_all_checkpoints() {
        ...
        create_test_checkpoint(3, 399)
            .save_to_canon(&mgr)
            .await
            .unwrap();
        create_test_checkpoint(1, 199)
            .save_to_canon(&mgr)
            .await
            .unwrap();
        create_test_checkpoint(2, 299)
            .save_to_canon(&mgr)
            .await
            .unwrap();
    ...
    async fn test_is_before_checkpoint() {
        ...
        assert!(MinerCheckpoint::is_before_checkpoint_multi(&mgr, 100)
            .await
            .unwrap());

        // Block 199 is at checkpoint
        assert!(MinerCheckpoint::is_before_checkpoint_multi(&mgr, 199)
            .await
            .unwrap());

        // Block 200 is after checkpoint
        assert!(!MinerCheckpoint::is_before_checkpoint_multi(&mgr, 200)
            .await
            .unwrap());
    }
}
