...
pub async fn detect_duplicate_canonical_blocks_multi(
    ...
    for block in canonical_blocks {
        blocks_by_index.entry(block.index).or_default().push(block);
    }
    ...
    for (index, blocks) in blocks_by_index {
        if blocks.len() > 1 {
            duplicates.push(DuplicateCanonicalBlock { index, blocks });
        }
    }
    ...
    for dup in duplicates {
        ...
        for block in &sorted_blocks[1..] {
            ...
            orphaned.mark_as_orphaned(
                format!(
                    "Duplicate canonical block at index {} - resolved via fork choice rules",
                    dup.index
                ),
...
mod tests {
    ...
    async fn test_detect_and_heal_duplicates() {
        ...
        // Heal
        let orphaned = heal_duplicate_canonical_blocks_multi(&ds, duplicates)
            .await
            .unwrap();
        ...
        let canonical = MinerBlock::find_canonical_by_index_simple(&ds, 1)
            .await
            .unwrap();
        ...
        let block_1b = MinerBlock::find_by_hash_multi(&ds, "hash_1b")
            .await
            .unwrap();
