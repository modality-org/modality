...
impl Model for DAGState {
    ...
    fn set_field(&mut self, field: &str, value: serde_json::Value) {
        match field {
            ...
            "certificate_count" => {
                self.certificate_count = value.as_u64().unwrap_or_default() as usize
            }
    ...
    fn get_id_keys(&self) -> HashMap<String, String> {
        ...
        keys.insert(
            "checkpoint_round".to_string(),
            self.checkpoint_round.to_string(),
        );
        keys
    }
}
    ...
    pub async fn get_latest_multi(datastore: &DatastoreManager) -> Result<Option<Self>> {
        ...
        for result in iterator {
            ...
            if let Some(round_str) = parts.get(4) {
                let keys = [("checkpoint_round".to_string(), round_str.to_string())]
                    .into_iter()
                    .collect();
