...
async fn test_static_validator_network_flow() -> Result<()> {
    ...
    assert!(
        stored_validators.is_some(),
        "Static validators should be stored"
    );
    ...
    assert_eq!(
        validator_set.nominated_validators, validator_set_epoch_1.nominated_validators,
...
async fn test_dynamic_validator_network_without_blocks() -> Result<()> {
    ...
    let stored_validators = datastore.get_static_validators().await?;
    assert!(
        stored_validators.is_none(),
        "No static validators should be stored"
    );
