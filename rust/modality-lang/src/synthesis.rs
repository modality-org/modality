...
use crate::ast::{FormulaExpr, Model, Part, Property, PropertySign, Transition};
...
pub fn synthesize_from_pattern(name: &str, pattern: &RulePattern) -> SynthesisResult {
    match pattern {
        ...
        RulePattern::Unknown => SynthesisResult::NeedsAssistance {
            partial: None,
            question: "Could not identify rule pattern. Please describe the desired behavior."
                .to_string(),
        },
    }
}
...
pub mod templates {
    ...
    pub fn handshake(party_a: &str, party_b: &str) -> Model {
        ...
        let mut t1 = Transition::new(
            "pending".to_string(),
            format!("{}_signed", party_a.to_lowercase()),
        );
        ...
        let mut t2 = Transition::new(
            "pending".to_string(),
            format!("{}_signed", party_b.to_lowercase()),
        );
    ...
    pub fn multisig(signers: &[&str], required: usize) -> Model {
        ...
        let signer_props: Vec<String> = signers
            .iter()
    ...
    pub fn milestone(client: &str, contractor: &str, milestones: &[&str]) -> Model {
        ...
        for milestone_name in milestones.iter() {
            let complete_state = format!(
                "{}_complete",
                milestone_name.to_lowercase().replace(' ', "_")
            );
            ...
            t_complete.add_property(Property::new(
                PropertySign::Plus,
                format!(
                    "COMPLETE_{}",
                    milestone_name.to_uppercase().replace(' ', "_")
                ),
            ));
            ...
            t_pay.add_property(Property::new(
                PropertySign::Plus,
                format!("PAY_{}", milestone_name.to_uppercase().replace(' ', "_")),
            ));
