...
impl ContractLog {
            ...
            for action in &commit.actions {
                match action {
                    ...
                    Action::Domain { properties } => {
                        state
                            .domain_history
                            .push((commit.commit_id, properties.clone()));
                    }
                    Action::Finalize => {
    ...
    pub fn validate_commit(
        &self,
        actions: &[Action],
        new_model: Option<&crate::ast::Model>,
    ) -> Result<(), String> {
...
impl Commit {
    ...
    pub fn adds_rules(&self) -> bool {
        self.actions
            .iter()
            .any(|a| matches!(a, Action::AddRule { .. }))
    }
}
...
mod tests {
    use super::*;
    use crate::ast::{FormulaExpr, Model, Part, PropertySign, Transition};
            ...
            vec![
                ...
                Action::AddRule {
                    ...
                    formula: Formula::new(
                        ...
                        FormulaExpr::Eventually(Box::new(FormulaExpr::Prop(
                            "delivered".to_string(),
                        ))),
                    ),
                },
    ...
    fn test_validate_with_custom_model() {
        ...
        let actions = vec![Action::AddRule {
        ...
        assert!(contract
            .validate_commit(&actions, Some(&simple_exchange_model()))
            .is_ok());
    }
    ...
    fn test_any_commit_can_update_model() {
        ...
        let state = contract.derive_state();
        assert_eq!(
            state.current_model.as_ref().unwrap().name,
            "UpdatedExchange"
        );
    }
}
