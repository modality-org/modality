...
use crate::ast::{Formula, Model};
use crate::lalrpop_parser::{
    parse_all_formulas_content_lalrpop, parse_all_models_content_lalrpop, parse_content_lalrpop,
};
use crate::mermaid::{
    generate_mermaid_diagram, generate_mermaid_diagram_with_state,
    generate_mermaid_diagram_with_styling,
};
...
impl ModalityParser {
    ...
    pub fn parse_model(&self, content: &str) -> Result<JsValue, JsValue> {
        let model = parse_content_lalrpop(content).map_err(|e| JsValue::from_str(&e))?;
        wasm_bindgen::JsValue::from_serde(&model)
    ...
    pub fn parse_all_models(&self, content: &str) -> Result<JsValue, JsValue> {
        let models =
            parse_all_models_content_lalrpop(content).map_err(|e| JsValue::from_str(&e))?;
        wasm_bindgen::JsValue::from_serde(&models)
            .map_err(|e| JsValue::from_str(&format!("Serde error: {}", e)))
    }
    ...
    pub fn parse_formulas(&self, content: &str) -> Result<JsValue, JsValue> {
        let formulas =
            parse_all_formulas_content_lalrpop(content).map_err(|e| JsValue::from_str(&e))?;
        wasm_bindgen::JsValue::from_serde(&formulas)
            .map_err(|e| JsValue::from_str(&format!("Serde error: {}", e)))
    }
    ...
    pub fn check_formula_any_state(
        &self,
        model_json: &str,
        formula_json: &str,
...
pub fn parse_all_models(content: &str) -> Result<JsValue, JsValue> {
    let models = parse_all_models_content_lalrpop(content).map_err(|e| JsValue::from_str(&e))?;
    wasm_bindgen::JsValue::from_serde(&models)
        .map_err(|e| JsValue::from_str(&format!("Serde error: {}", e)))
}
