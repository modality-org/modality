...
mod tests {
    ...
    #[test]
    fn test_finance_party_roles() {
        let result = map_nl_to_pattern(
            "Bank settles transfer after account holder, cardholder, card issuer, and payment processor approve charge",
        );
        assert!(result.parties.contains(&"Bank".to_string()));
        assert!(result.parties.contains(&"AccountHolder".to_string()));
    ...
    #[test]
    fn test_real_estate_party_roles() {
        let result = map_nl_to_pattern(
            "Landlord transfers keys after tenant, realtor, property manager, title company, and escrow officer approve closing",
        );
        assert!(result.parties.contains(&"Landlord".to_string()));
        assert!(result.parties.contains(&"Tenant".to_string()));
        assert!(result.parties.contains(&"Realtor".to_string()));
    ...
    #[test]
    fn test_audit_party_roles() {
        let result = map_nl_to_pattern(
            "Auditor files attestation after auditee, compliance officer, certification body, and audit committee approve controls",
        );
        assert!(result.parties.contains(&"Auditor".to_string()));
        assert!(result.parties.contains(&"Auditee".to_string()));
        assert!(result.parties.contains(&"ComplianceOfficer".to_string()));
        assert!(result.parties.contains(&"CertificationBody".to_string()));
        assert!(result.parties.contains(&"AuditCommittee".to_string()));
    }

    #[test]
    fn test_kyc_party_roles() {
        let result = map_nl_to_pattern(
            "Relying party accepts onboarding after identity provider, KYC provider, and beneficial owner approve verification",
        );
        assert!(result.parties.contains(&"RelyingParty".to_string()));
