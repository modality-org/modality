use crate::ast::{Action, ActionCall, Contract, Formula, Model, Test};
use crate::grammar::{
    ActionCallParser, ActionParser, ContractDeclParser, FormulaParser, TopLevelParser,
};
use std::fs;
use std::path::Path;
...
pub fn parse_rule_for_this_commit_content(
    content: &str,
) -> Result<crate::ast::RuleForThisCommit, String> {
...
"#;
        ...
        match &formula.expression {
            ...
            _ => panic!(
                "Expected unlabeled DiamondBox, got {:?}",
                formula.expression
            ),
        }
    }
...
"#;
        ...
        match &formula.expression {
            FormulaExpr::Always(inner) => match &**inner {
    ...
    fn test_diamond_box_expansion() {
        ...
        let props = vec![crate::ast::Property::new(
            PropertySign::Plus,
            "PAY".to_string(),
        )];
...
"#;
        ...
        match &formulas[0].expression {
            FormulaExpr::Box(props, _) => {
                assert!(
                    props.is_empty(),
                    "Expected empty properties for unlabeled box"
                );
            }
...
"#;
        ...
        match &formulas2[0].expression {
            FormulaExpr::Diamond(props, _) => {
                assert!(
                    props.is_empty(),
                    "Expected empty properties for unlabeled diamond"
                );
            }
    ...
    fn test_desugar_temporal_to_fixpoint() {
            ...
            FormulaExpr::Gfp(var, inner) => {
                ...
                match *inner {
                    FormulaExpr::And(box_part, prop_part) => {
                        assert!(
                            matches!(*box_part, FormulaExpr::Box(props, _) if props.is_empty())
                        );
        ...
        let eventually_goal =
            FormulaExpr::Eventually(Box::new(FormulaExpr::Prop("goal".to_string())));
            ...
            FormulaExpr::Lfp(var, inner) => {
                ...
                match *inner {
                    FormulaExpr::Or(diamond_part, prop_part) => {
                        assert!(
                            matches!(*diamond_part, FormulaExpr::Diamond(props, _) if props.is_empty())
                        );
...
"#;
        ...
        match &rule.expression {
            CommitRuleExpr::And(left, right) => {
                assert!(
                    matches!(left.as_ref(), CommitRuleExpr::SignedBy(s) if s == "/users/alice.id")
                );
                assert!(
                    matches!(right.as_ref(), CommitRuleExpr::SignedBy(s) if s == "/users/bob.id")
                );
            }
