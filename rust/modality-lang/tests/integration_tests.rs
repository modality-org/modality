...
use modality_lang::ast::{Property, PropertySign, Transition};
use modality_lang::evolution::{Amendment, EvolvableContract};
...
fn test_contract_upgrade_to_new_model() {
    ...
    let mut evolvable =
        EvolvableContract::new(old_model, vec!["Alice".to_string(), "Bob".to_string()], 2);
...
use modality_lang::lalrpop_parser::{parse_all_formulas_content_lalrpop, parse_content_lalrpop};
...
fn test_membership_growth_model_evolution() {
    ...
    contract
        .post(
            ...
            PathValue::PubKey(alice_key.to_string()),
        )
        .unwrap();
    ...
    contract
        .post(
            "/members/bob.pubkey",
            PathValue::PubKey(bob_key.to_string()),
        )
        .unwrap();
    ...
    contract
        .post(
            "/members/carol.pubkey",
            PathValue::PubKey(carol_key.to_string()),
        )
        .unwrap();
