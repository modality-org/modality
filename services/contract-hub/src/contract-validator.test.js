...
test('RULE commits accept satisfying witnessModel aliases', () => {
  const validator = new ContractValidator();
  const ruleCommit = {
    data: {
      method: 'RULE',
...
test('validateContractLogic applies parser-backed must macro rules within a batch', async () => {
  const store = {
    pullCommits() {
      return [];
    }
  ...
  const ruleCommit = {
    data: {
      method: 'RULE',
      path: '/rules/required-release.modality',
      content: 'rule required_release { formula { always (must(+RELEASE)) } }',
      model: `
        model required_release_witness {
          initial active
          active -> active [+RELEASE]
  ...
  assert.equal(validReplacement.valid, true);
  assert.equal(validReplacement.state.model.name, 'required_release_ok');
});

test('validateContractLogic applies parser-backed negated can macro rules within a batch', async () => {
  const store = {
    pullCommits() {
      return [];
    }
  ...
  const ruleCommit = {
    data: {
      method: 'RULE',
      path: '/rules/no-release.modality',
      content: 'rule no_release { formula { always (not can(+RELEASE)) } }',
      model: `
        model no_release_witness {
          initial active
          active -> active [-RELEASE]
...
test('validateContractLogic applies parser-backed box constant rules within a batch', async () => {
  const store = {
    pullCommits() {
      return [];
    }
...
test('validateContractLogic applies parser-backed diamond constant rules within a batch', async () => {
  const store = {
    pullCommits() {
      return [];
    }
...
test('validateContractLogic applies parser-backed empty box rules within a batch', async () => {
  ...
  const ruleCommit = {
    data: {
      method: 'RULE',
      path: '/rules/always-owner.modality',
      content: 'rule always_owner { formula { always ([] signed_by(/owner.id)) } }',
      model: `
        model always_owner_witness {
          initial active
          active -> active [+signed_by(/owner.id)]
...
test('validateContractLogic accepts tautological empty modal rules without witnesses', async () => {
  const store = {
    pullCommits() {
      return [];
    }
