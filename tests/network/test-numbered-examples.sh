#!/usr/bin/env bash
# Test runner for numbered network examples only
# Dynamically discovers and runs examples matching pattern [0-9]*-*
...
echo "================================"
echo "Numbered Network Examples Tests"
echo "================================"
echo ""
echo "Configuration:"
...
# Categorize examples by speed
# These mappings can be updated as needed
declare -A QUICK_EXAMPLES=(
    ["01-ping-node"]=1
    ["02-run-devnet1"]=1
    ["04-sync-miner-blocks"]=1
    ["06-contract-lifecycle"]=1
    ["07-contract-assets"]=1
    ["08-network-partition"]=1
    ["09-network-parameters"]=1
)

declare -A SLOW_EXAMPLES=(
    ["05-mining"]=1
)

# Dynamically discover numbered examples
echo "Discovering numbered examples..."
NUMBERED_EXAMPLES=()
for dir in [0-9]*-*/; do
    if [ -d "$dir" ]; then
        # Remove trailing slash
        example_name="${dir%/}"
        NUMBERED_EXAMPLES+=("$example_name")
    fi
done

# Sort examples numerically
IFS=$'\n' NUMBERED_EXAMPLES=($(sort -V <<<"${NUMBERED_EXAMPLES[*]}"))
unset IFS

echo "Found ${#NUMBERED_EXAMPLES[@]} numbered examples:"
for example in "${NUMBERED_EXAMPLES[@]}"; do
    echo "  - $example"
done
echo ""

# Run all discovered numbered examples
for example in "${NUMBERED_EXAMPLES[@]}"; do
    # Determine category
    category="normal"
    if [[ -n "${QUICK_EXAMPLES[$example]}" ]]; then
        category="quick"
    elif [[ -n "${SLOW_EXAMPLES[$example]}" ]]; then
        category="slow"
    fi
    
    # Format name for display
    display_name=$(echo "$example" | sed 's/-/ - /' | sed 's/\(^[0-9]*\)/\1/')
    
    run_test_suite "$example" "$display_name" "$category"
done
