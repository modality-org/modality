...
- `check-rule-synthesize-cli.sh` - Real CLI smoke for synthesizing a signed-`POST` witness model from a rule formula block with `--verify` when a `modality` binary is available
