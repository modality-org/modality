...
FAKE_OVERLONG_REV_MODALITY="$(mktemp)"
cat >"$FAKE_OVERLONG_REV_MODALITY" <<EOF
#!/usr/bin/env bash
if [[ "\${1:-}" == "--version" ]]; then
  echo "modality 0.0.0 (@$overlong_revision_marker)"
  exit 0
fi
echo "overlong-revision modality test binary should only be asked for --version" >&2
exit 1
EOF
chmod 0755 "$FAKE_OVERLONG_REV_MODALITY"
negative_output="$(
  MODAL_ONBOARDING_ARTIFACT_SMOKE=1 \
  MODAL_ONBOARDING_ARTIFACT_EXPECT_REV="${MODAL_ONBOARDING_ARCHIVE_EXPECT_REV:-}" \
  MODALITY_BIN="$FAKE_OVERLONG_REV_MODALITY" \
    "$ROOT_DIR/tests/cli/check-modal-release-artifact-download.sh" "$ARCHIVE_DIR" 2>&1
)" && {
  echo "release artifact verifier accepted a modality smoke binary with an overlong revision" >&2
  exit 1
}
if ! grep -Fq "release artifact smoke modality version revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release artifact verifier rejected the overlong modality smoke revision for the wrong reason
expected: release artifact smoke modality version revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
long_expected_revision="${source_revision}0"
negative_output="$(
  MODAL_ONBOARDING_ARTIFACT_EXPECT_REV="$long_expected_revision" \
    "$ROOT_DIR/tests/cli/check-modal-release-artifact-download.sh" "$ARCHIVE_DIR" 2>&1
)" && {
  echo "release artifact verifier accepted an overlong expected revision" >&2
  exit 1
}
if ! grep -Fq "release artifact expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release artifact verifier rejected the overlong expected revision for the wrong reason
expected: release artifact expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
nonhex_expected_revision="${source_revision:0:6}g"
negative_output="$(
  MODAL_ONBOARDING_ARTIFACT_EXPECT_REV="$nonhex_expected_revision" \
    "$ROOT_DIR/tests/cli/check-modal-release-artifact-download.sh" "$ARCHIVE_DIR" 2>&1
)" && {
  echo "release artifact verifier accepted a non-hex expected revision" >&2
  exit 1
}
if ! grep -Fq "release artifact expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release artifact verifier rejected the non-hex expected revision for the wrong reason
expected: release artifact expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
negative_output="$(
  MODAL_ONBOARDING_ARTIFACT_EXPECT_REV="unknown" \
    "$ROOT_DIR/tests/cli/check-modal-release-artifact-download.sh" "$ARCHIVE_DIR" 2>&1
)" && {
  echo "release artifact verifier accepted an unknown expected revision" >&2
  exit 1
}
if ! grep -Fq "release artifact expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release artifact verifier rejected the unknown expected revision for the wrong reason
expected: release artifact expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
negative_output="$(
  ...
  echo "release archive readiness accepted an overlong producer expected revision" >&2
  exit 1
}
if ! grep -Fq "release archive expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release archive readiness rejected overlong producer expected revision for the wrong reason
expected: release archive expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
negative_output="$(
  MODAL_ONBOARDING_ARCHIVE_EXPECT_REV="$nonhex_expected_revision" \
    "$ROOT_DIR/tests/cli/check-modal-release-archive-readiness.sh" 2>&1
)" && {
  echo "release archive readiness accepted a non-hex producer expected revision" >&2
  exit 1
}
if ! grep -Fq "release archive expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release archive readiness rejected non-hex producer expected revision for the wrong reason
expected: release archive expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
...
negative_output="$(
  MODAL_ONBOARDING_ARCHIVE_EXPECT_REV="unknown" \
    "$ROOT_DIR/tests/cli/check-modal-release-archive-readiness.sh" 2>&1
)" && {
  echo "release archive readiness accepted an unknown producer expected revision" >&2
  exit 1
}
if ! grep -Fq "release archive expected source revision is not a lowercase hex commit token" <<<"$negative_output"; then
  cat >&2 <<EOF
release archive readiness rejected unknown producer expected revision for the wrong reason
expected: release archive expected source revision is not a lowercase hex commit token
actual:
$negative_output
EOF
