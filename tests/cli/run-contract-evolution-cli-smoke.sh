...
grep -q 'Add signed-post rule' "$TMP_DIR/log.txt"
grep -q 'Accept Bob in V2' "$TMP_DIR/log.txt"
grep -q 'Bob signed V2 update' "$TMP_DIR/log.txt"
grep -q "$ALICE_ID" "$TMP_DIR/log.json"
grep -q "$BOB_ID" "$TMP_DIR/log.json"
grep -q "$ALICE_ID" "$TMP_DIR/log.txt"
grep -q "$BOB_ID" "$TMP_DIR/log.txt"
...
"$MODAL_BIN" c status --dir "$MEMBERS_CONTRACT_DIR" >"$TMP_DIR/members-status.txt"
"$MODAL_BIN" c log --dir "$MEMBERS_CONTRACT_DIR" >"$TMP_DIR/members-log.txt"

grep -q '"total_commits": 6' "$TMP_DIR/members-status.json"
grep -q '"model_state": "active"' "$TMP_DIR/members-status.json"
grep -q 'Model state: active' "$TMP_DIR/members-status.txt"
...
grep -q 'Alice ordinary update' "$TMP_DIR/members-log.txt"
grep -q 'Alice adds Bob' "$TMP_DIR/members-log.txt"
grep -q 'Bob ordinary update' "$TMP_DIR/members-log.txt"
grep -q 'Alice and Bob replace model' "$TMP_DIR/members-log.txt"
grep -q "$ALICE_ID" "$TMP_DIR/members-log.json"
grep -q "$BOB_ID" "$TMP_DIR/members-log.json"
grep -q "$ALICE_ID" "$TMP_DIR/members-log.txt"
grep -q "$BOB_ID" "$TMP_DIR/members-log.txt"
...
"$MODAL_BIN" c status --dir "$BOUNDED_CONTRACT_DIR" >"$TMP_DIR/bounded-status.txt"
"$MODAL_BIN" c log --dir "$BOUNDED_CONTRACT_DIR" >"$TMP_DIR/bounded-log.txt"

grep -q '"total_commits": 6' "$TMP_DIR/bounded-status.json"
grep -q '"model_state": "expired"' "$TMP_DIR/bounded-status.json"
grep -q 'Model state: expired' "$TMP_DIR/bounded-status.txt"
...
grep -q 'Add accumulated expiry-shape rule' "$TMP_DIR/bounded-log.txt"
grep -q 'Add delivery bounded term' "$TMP_DIR/bounded-log.txt"
grep -q 'Record completion evidence' "$TMP_DIR/bounded-log.txt"
grep -q 'Unsigned release after completion' "$TMP_DIR/bounded-log.txt"
