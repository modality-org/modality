...
required_patterns=(
  ...
  "replayed accepted-state oracle-key map"
  "derive \`accepted_state_oracle_keys\`"
  "actual accepted contract state"
  "\`/oracles/**/*.id\` paths"
  "after updates and deletes are applied"
  "pending commit is expanded or checked"
  "WASM program"
  "\`context.accepted_state_oracle_keys\`"
  "replayed invoke programs can build oracle replay bundles"
  "accepted-state key material the verifier will check"
  "derives the oracle key from \`expected_oracle_path\`"
  "valid hex-encoded ed25519 public-key material"
  "scalar \`expected_oracle_pubkey\` agrees"
  "Missing replay-bundle freshness"
  "policy,"
  "bundle/input"
  "freshness mismatches"
  "missing accepted-state oracle-key lookup maps or path entries"
  "accepted-state oracle-key material"
  "scalar/key-map"
  "oracle-key mismatches"
  ...
  "predicate names, and"
  "envelope/input"
  "mismatches fail before"
  "signature"
  "verification"
  "This"
  "extension-level"
  "contract-log validator passes the replay bundle"
  "oracle predicate evaluator"
  "accepted-state oracle-key map"
...
wasm_patterns=(
  ...
  "the predicate input"
  "\`accepted_state_oracle_keys\`"
  "replayed accepted-state oracle-key"
  "derive"
  "\`accepted_state_oracle_keys\` from the actual accepted contract state"
  "\`/oracles/**/*.id\` paths"
  "after updates and deletes are"
  "pending commit is expanded or checked"
  "WASM program context now carries"
  "\`context.accepted_state_oracle_keys\`"
  "replayed invoke programs can build"
  "same accepted-state key material"
  "derives the key at \`expected_oracle_path\`"
  "valid hex-encoded ed25519 public-key material"
  "scalar \`expected_oracle_pubkey\` agrees"
  "missing accepted-state oracle-key lookup maps or path entries"
  "malformed accepted-state oracle keys"
  "scalar/key-map mismatches"
  "accepted-state oracle"
  "key mismatches"
  "rejects missing"
  "replay-bundle freshness policies, bundle/input"
  "mismatches, missing accepted-state oracle-key lookup maps"
  "key mismatches, malformed"
  "bundle/input"
  "bundle/input freshness"
  "malformed JSON,"
  "non-canonical JSON bytes, wrong predicate"
  "names, and attestations that"
  "attestations that differ"
  "from the predicate input before an oracle"
  "claim can pass"
