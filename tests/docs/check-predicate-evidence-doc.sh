...
required_patterns=(
  ...
  "wrong predicate names, and envelope/input"
  "attestation mismatches fail before signature verification"
  "until a contract-log validator supplies the"
  "bundle from replay data and looks up the accepted-state oracle key"
  "### post_to_path"
...
wasm_patterns=(
  ...
  "rejects malformed JSON, non-canonical JSON bytes, wrong predicate names, and"
  "attestations that differ from the predicate input"
  "rejects missing oracle-path"
  "paths, missing pending-commit bindings, and mismatched pending commit hashes"
