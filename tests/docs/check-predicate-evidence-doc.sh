...
required_patterns=(
  ...
  "positive \`max_age_seconds\` freshness policy"
  ...
  "signature"
  "verification"
  "This is still extension-level evidence only"
  "validator supplies the bundle from replay data"
  "looks up the accepted-state"
...
wasm_patterns=(
  ...
  "requires a positive \`max_age_seconds\` freshness policy"
  "rejects missing replay-bundle freshness policies, malformed JSON,"
  "non-canonical JSON bytes, wrong predicate names, and"
  "and attestations that differ"
  "from the predicate input before an oracle claim can pass"
  "rejects missing oracle-path"
  "mismatched oracle paths, missing"
  "pending-commit bindings, and mismatched pending commit hashes"
  "mismatched pending commit hashes"
  "accepted-state oracle-key lookup"
  "freshness policy"
