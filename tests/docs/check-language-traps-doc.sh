...
AGENT_TASK_EXCHANGE_EXAMPLE="$ROOT_DIR/examples/agent_task_exchange.modality"
...
for pattern in "${agent_task_exchange_example_required_patterns[@]}"; do
  if ! grep -Fq "$pattern" "$AGENT_TASK_EXCHANGE_EXAMPLE"; then
    echo "agent task exchange example is missing language-trap text: $pattern" >&2
    exit 1
  fi
done

if grep -Eq -- 'claimed[[:space:]]*->[[:space:]]*eventually| implies ' "$AGENT_TASK_EXCHANGE_EXAMPLE"; then
  echo "agent task exchange example should not present formula implication sugar as the teaching path" >&2
  exit 1
fi
...
dev_concepts_required_patterns=(
  "Status: archived concept notes."
  "avoid formula implication sugar such as \`A -> B\`"
  "explicit Boolean conditionals such as"
  "avoid \`[+ACTION] true\` as a conditional antecedent"
  "always(!<+RELEASE> true | <+DELIVER> true)"
  "This uses explicit Boolean form for the conditional"
)

for pattern in "${dev_concepts_required_patterns[@]}"; do
  if ! grep -Fq "$pattern" "$DEV_CONCEPTS_DOC"; then
    echo "developer concepts doc is missing language-trap text: $pattern" >&2
    exit 1
  fi
done

if grep -Eq -- ' true[[:space:]]*->| implies ' "$DEV_CONCEPTS_DOC"; then
  echo "developer concepts doc should not present formula implication sugar as the teaching path" >&2
  exit 1
fi

dev_model_synthesis_required_patterns=(
  ...
  "always(!<+A> true | <+A +required> true)"
  "!<+RELEASE> true | <+DELIVER> true"
  "<+EXECUTE +signed_by(/users/alice.id) +signed_by(/users/bob.id)> true"
)

for pattern in "${dev_model_synthesis_required_patterns[@]}"; do
  if ! grep -Fq "$pattern" "$DEV_MODEL_SYNTHESIS_TUTORIAL"; then
    echo "developer model synthesis tutorial is missing language-trap text: $pattern" >&2
    exit 1
  fi
done
