...
required_patterns=(
  ...
  "Alice and Bob together can replace the witness"
  "repeated \`--sign\` flags"
  ...
  "/terms/delivery_complete.bool"
  "+bool_true(/terms/delivery_complete.bool)"
  "rejecting the guarded move before completion evidence"
  "accepting the same"
  "after completion evidence"
  'ending replay in the `expired`'
  'appends an older `eventually(expired)` rule first'
  "rejects a later witness replacement that removes the expiry state"
