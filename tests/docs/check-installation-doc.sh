...
required_patterns=(
  ...
  "treats an explicitly set but non-executable \`MODALITY_BIN\` as an"
  "error instead of archive-only evidence"
  ...
  "summary records the exact Actions artifact name, the GitHub artifact"
  "digest emitted by the \`upload-artifact\` \`artifact-digest\` output"
  "fails if that digest is missing"
  ...
  "matching workflow run id"
  "exact Actions artifact name"
  "GitHub artifact digest"
  "detached tarball checksum line"
  "downloaded-artifact verifier result"
  "when first-contract replay is claimed"
  "same-revision \`MODALITY_BIN\` smoke replay result"
  "modal-v*"
  ...
  "regular non-symlink \`bin/\`"
  "directory with mode \`0755\`"
  "\`bin/\` and \`bin/modal\` as"
  "\`bin/modal\` as"
  "\`0755\`"
  ...
  "provenance"
  "advertised help surface to match the wrapper feature set"
  ...
  "\`modal --version\` marker cannot become"
  "self-consistent archive provenance"
  "version provenance as installer metadata"
  "extra hand-written version"
  ...
  "downloaded \`bin/modal --version\` output to be one line"
  "matches provenance before help-surface or first-contract replay is trusted"
  "return successfully with one line"
  "identifies the language CLI with a \`modality\` prefix"
  ...
  "\`modality-cli-contract\`, \`modality-common\`, and \`modality\`"
  "selected package"
  "\`modality-cli-common\`, \`modality-cli-contract\`, \`modality-common\`, \`modality\`"
  "and \`modality-lang\`"
...
required_workflow_patterns=(
  ...
  "Actions artifact:"
  "GitHub artifact digest:"
  "steps.upload-archive.outputs['artifact-digest']"
  ...
  "expected upload-artifact to report a GitHub artifact digest"
  "\${upload_digest}"
  "binary version, recorded help surface, same-revision language CLI, first-contract smoke"
  "Smoke replay:"
  "### Local replay"
...
if ! grep -Fq 'release archive readiness accepted a too-short producer expected revision' \
  "$ROOT_DIR/tests/cli/check-modal-release-archive-readiness.sh"; then
  echo "release archive producer should prove too-short producer expected revisions are rejected" >&2
  exit 1
fi
...
if ! grep -Fq 'release artifact verifier accepted a non-hex expected revision' \
  "$ROOT_DIR/tests/cli/check-modal-release-archive-readiness.sh"; then
  echo "release archive producer should prove non-hex artifact expected revisions are rejected" >&2
  exit 1
fi
