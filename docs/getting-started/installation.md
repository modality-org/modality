...
return one line that identifies the language CLI with a `modality` prefix, so a
helper that only prints a matching source revision marker or appends extra
