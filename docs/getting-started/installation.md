...
the same version, profile, and help surface as provenance, so stale human-facing
bundle notes fail before the binary is trusted. Those README metadata fields
must also be single-valued, so hand-edited notes with both current and stale
values fail before the binary is trusted.
