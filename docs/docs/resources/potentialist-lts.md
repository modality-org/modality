...
Common predicates:
- $\mathsf{signed\_by}(\textit{path})$ — cryptographic signature
- $\mathsf{threshold}(n, \textit{path})$ — $n$-of-$m$ multisig
- $\mathsf{before}(\textit{time})$ — temporal deadline
- $\mathsf{oracle}(\textit{source}, \textit{claim})$ — external attestation

A transition $s \xrightarrow{+\mathsf{signed\_by}(\texttt{/users/alice})} s'$ requires Alice's signature to execute.

### 8.3 Label Satisfaction

An action $\alpha$ **satisfies** label $a = +p(\text{args})$ iff predicate $p$ evaluates to true on $\alpha$'s context. It satisfies $-p(\text{args})$ iff $p$ evaluates to false.
...
- $\mathcal{L}_0$: single state $\texttt{init}$
- $\Gamma_0 = []$

Alice adds covenant:

$$
\phi_A = \mathsf{starting\_at}(\texttt{init}, \mathsf{eventually}(\texttt{done}))
$$
...
$$
\phi_B = \mathsf{starting\_at}(\texttt{init}, \mathsf{eventually}(\texttt{done}))
$$

Same formula — the potential space now requires reaching $\texttt{done}$.

Execution: Alice signs (→ $\texttt{a\_ready}$), Bob signs (→ $\texttt{done}$). Both covenants satisfied.
...
$$
\mathsf{always}([\texttt{release}] \Rightarrow \langle\texttt{deliver}\rangle\top)
$$
...
$$
@_{\texttt{disputed}}([\texttt{release}] \Rightarrow \langle\texttt{deliver}\rangle\top)
$$
...
$$
\mathsf{always}([\texttt{+withdraw}] \Rightarrow \langle\texttt{+signed\_by(alice)}\rangle\top)
$$
